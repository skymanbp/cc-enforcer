#!/usr/bin/env python3
"""cc-enforcer — Stop hook entry point.

A thin shell (v0.41). The hook's body is `stop_guard_impl.py`, the module
...
asked to run — `python stop_guard.py` re-compiled all ~2,000 lines of the
body on every Stop, about 7 ms on the maintainer's machine, more than the
hook's own work — while a module it *imports* is compiled once and served
from `__pycache__` for as long as the source is unchanged. Moving the body
one import away is the whole trick; nothing in the body changed.
...
so `stop_guard.X` is the very object the hook runs with: a test that
...
import stop_guard_impl as _impl  # noqa: E402 — because the sibling directory must be on sys.path first
