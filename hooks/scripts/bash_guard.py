#!/usr/bin/env python3
"""cc-enforcer — PreToolUse(Bash) hook entry point.

A thin shell (v0.41). The hook's body is `bash_guard_impl.py`, the module
imported below; `hooks.json` keeps pointing at this file, and so do the
tests, the benchmark and the docs.

Why the split: Python never caches the bytecode of the script it was
asked to run — `python bash_guard.py` re-compiled all ~640 lines of the
body on every Bash call, about 2 ms on the maintainer's machine — while a
module it *imports* is compiled once and served from `__pycache__` for as
long as the source is unchanged. Moving the body one import away is the
whole trick; nothing in the body changed.
...
so `bash_guard.X` is the very object the hook runs with: a test that
...
import bash_guard_impl as _impl  # noqa: E402 — because the sibling directory must be on sys.path first
