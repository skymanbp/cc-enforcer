...
This module is the hook's body. The entry `hooks.json` runs is the thin
`stop_guard.py` beside it, which imports this module and calls `main()`;
its docstring says why the two are split (v0.41).
...
laziness signals appear in proximity to a "done" claim:
  ...
  (a) [v0.6.0; evidence set narrowed v0.41] No convergence evidence at
      all: no `$ ` shell prompt, no command output, no test counts, no
      fenced code block. Pure "已解决" / "fixed" walking away — and
      "verified" / "已验证" walking away too: a word that names
      verification is a claim, not evidence, in either language.
...
EVIDENCE_PATTERNS = [
    ...
    # v0.41 — no vocabulary counts as evidence any more. Until then the
    # rule-06 check names (重触发 / 边界用例 / 反向用例 / 收敛) and three
    # English verbs (verified / re-ran / validated) sat in this list, so a
    # reply could clear the "show your work" layer by naming the work:
    # `收敛:` alone passed here in Chinese while `convergence:` did not,
    # and `Fixed, verified.` passed in English while `已修复，已验证。`
    # did not. Evidence is what output looks like — a prompt line, a
    # count, a runner's name, a fenced block — in either language. The
    # check names still count for layer (c), where naming the check is
    # exactly what is asked (CONVERGENCE_MARKERS below).
