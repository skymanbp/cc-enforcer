...
- **Thin entries** — each registered script is a shell of a few lines over a
  sibling `<name>_impl.py` module that holds the body. CPython caches the
  bytecode of a module it *imports* under `__pycache__` and re-uses it while
  the source is unchanged, but compiles the script it was asked to *run* from
  source on every start; with the body one import away, a hook no longer
  re-compiles its own one to two thousand lines per call (`stop_guard`: about
  7 ms of a hook whose own work is less than that). Imported rather than run,
  an entry hands back the body module itself, so `stop_guard.X` is the object
  the hook runs with and a patch through the entry's name reaches the running
  code. `tests/test_startup_cost.py` pins both halves: the body's `.pyc`
  exists after one run of the entry, the entry's never does, and the body
  run as a script is not cached either.
...
| 2 | (a) | 06 | no evidence (`EVIDENCE_PATTERNS`): a shell-prompt line (`$ `, `> `, `PS C:\…>`, `C:\…>`), `Ran N tests`, `N passed/failed`, `pytest` / `unittest`, or a fenced block of ≥ 20 characters. Vocabulary is not evidence in either language: a `verified` or a `重触发` names the work without showing it (the check names count for layer (c) instead). |
...
| `hooks/scripts/bash_guard.py` (entry) · `bash_guard_impl.py` (body) | `hooks/hooks.json` (matcher), §2.4, `lib/shellcmd.py`, `lib/messages*.py` (the deny texts), `tests/test_bash_guard.py` (a positive and a nearby negative for every pattern; register-flow cases), `tests/test_startup_cost.py` (the import inventory and the thin-shell pins). |
...
| `hooks/scripts/inject_context.py` (entry) · `inject_context_impl.py` (body) | `hooks/hooks.json`, `prompts/*.md` (both tiers, both languages), §2.2, `tests/test_inject_context.py` (content, budget and constraint-completeness gates), `tests/test_startup_cost.py` (the import inventory and the thin-shell pins). |
...
| `hooks/scripts/read_guard.py` (entry) · `read_guard_impl.py` (body) | `hooks/hooks.json`, `lib/state.py`, `lib/srclex.py`, `lib/editscale.py`, `lib/messages*.py`, §2.3, `rules/09` / `10` / `11` (the rules it enforces, both languages), `prompts/*.md` (the DENY rows), `tests/test_read_guard.py` (every allow with its deny twin), `tests/test_startup_cost.py` (the import inventory and the thin-shell pins). |
...
| `hooks/scripts/stop_guard.py` (entry) · `stop_guard_impl.py` (body) | `hooks/hooks.json`, `lib/state.py` (grace helpers, `did_edit_this_turn`), `lib/mdctx.py`, `lib/sync_gate.py`, `lib/messages*.py`, §2.5, `prompts/*.md` (the layer rows), `README*.md` (the nine-layer table and the hedge examples), `tests/test_stop_guard.py` (both directions for every pattern), `tests/test_startup_cost.py` (nothing may compile at import; the thin-shell pins). |
...
| `hooks/scripts/lib/editscale.py` | `read_guard.py` (the frequency layer formats the live constants), `rules/09` (+ zh: the thresholds in its "Enforced by" line), `prompts/*.md`, `README*.md` (the coverage-bar samples are derived from it), `tests/test_editscale.py`, `tests/test_read_guard.py`. |
...
| `rules/<nn>-*.md` | `rules/zh/<nn>-*.md`, `rules/00-index.md` (+ zh), `prompts/*.md`, `docs/RULES.md`, `commands/checklist.md`, `tests/test_inject_context.py`. Each rule's one "Enforced by" line under its title is the only place the rule describes a hook, and it restates §2; `rules/03` and `rules/09` (+ zh) must keep naming the whole Bash deny set there (`tests/test_doc_sync.py`). |
