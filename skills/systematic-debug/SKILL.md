...
- **因果链必须爬到最上游**（[`rules/03-root-cause.md`](rules/03-root-cause.md) 上游阶梯）：症状位 → 传播路径 → 起源；修在前两级都算补丁。若停在中途（起源在 vendor / 平台 / 别的仓库），显式点名真正起源 + 停下的理由。
...
- **统一修复**（[`rules/09-systematic-modification.md`](rules/09-systematic-modification.md) "One root cause, one unified fix"）：确诊的根因定义一个"类"——全库 Grep 同类实例、Read 确认，一趟修完；**严禁**对共享同一根因的多处症状逐个点对点修补。类实质性超出用户请求范围时，先上呈枚举结果再动手。
- 对每个连带项也同步修改。
- 修改时禁止规则 03 列出的反模式。
...
**7.2 · 边界 + 反向用例**：至少跑 1 个边界（空输入 / 错误路径 / 并发 / 跨平台 / Unicode）+ 1 个反向用例（应该 fail 的仍 fail）。类不止一个实例时，还要重触发类里**另一个**实例证明类已闭合；只有一个实例则显式说明，以 Step 6 的清扫报告为闭合证据。
...
- ❌ 对共享同一根因的多处症状逐个点对点修补 → 先诊断共同起源，再一次统一修复（规则 09）
