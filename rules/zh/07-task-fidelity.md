...
**强制执行：** `Stop` layer (d) —— 完成声明既无忠实标记（`rule 07` / `task fidelity` / `request coverage` / `no scope creep` / `任务忠实` / `原始请求` …）、又答不足下面三问中的两问，即 BLOCK。拆解请求本身是文本层纪律。
...
**最小有效更改**，在此定义一次：能以用户要求的标准完整解决请求的最小改动集——请求不需要的一律不做（不顺手重构、改名、抽象或重排格式），修到根因所需的一样不少（rule 03）。目标不是"小"，而是"够用且不多"。
