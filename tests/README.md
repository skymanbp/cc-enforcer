...
| [`test_stop_guard.py`](test_stop_guard.py) | 151 | [`stop_guard.py`](../hooks/scripts/stop_guard.py) — all nine layers, the status-table format contract, per-layer grace, production-shape payloads (no `turn_count`), transcript fallback, tldr presence and length. v0.39.2 adds `TestTranscriptTailRead`: the fallback reads the transcript's tail in growing windows, so a 60 MB file costs under four windows (tracemalloc), a line longer than the window grows it, and 256-byte windows agree with a whole-file scan. v0.35 adds `TestTldrDisplayWidth`: the cap counts display columns, so 100 CJK characters (200 columns) now block while 75 (150 columns) still pass and every ASCII boundary is unmoved — the strictness increase and the no-op asserted in both directions. |
