...
[![Plugin Version](https://img.shields.io/badge/version-0.39.2-blue.svg)](CHANGELOG.md)
...
| 06 | **收敛验证** | 重触发原症状、跑边界与反向用例、跑既有测试，回答四道自答题。「无回归」必须是逐项集合差，不能是总数相等。 | **Stop (a)(c)** |
...
（无上界的）圣旨列表是让步的一方，按**整条圣旨**的边界省略并留一个指针——连契约
本身都填满预算时，注入里也会说出丢了几条。一条测试用 120 字符的安装根加三条真实
圣旨对四份提示词逐一验证不发生省略，预算不会再一版一版地被蚕食。

每个钩子都经 [`lib/hookio.py`](hooks/scripts/lib/hookio.py) 按 UTF-8 严格解码载荷，
...
你会失去硬层（那是 Claude Code 钩子），保留推理纪律。整包太长塞不进 system prompt
时，[`rules/zh/00-index.md`](rules/zh/00-index.md) 是短版；规则包这一层的说明见
...
两张图都可以用 `python demo/run_demo.py --svg` 现场复现，源码与完整说明在
[`demo/`](demo/README.md)。
...
图里每一条 cc-enforcer 判决都是钩子的**逐字输出**；agent 的那五步是**脚本化**的，
正是脚本化才让两遍在除钩子之外的一切上完全相同。
...
这里用的分类：
  ...
  系统式  = 最大字符数 ≥ 1500 或 最大行数 ≥ 50
            或 该改动跨越了本文件的 ≥ 30% —— 这里是 37/122 行，或 1102/3672 字符
            （把计数器清 0）
  中等    = 介于两者之间（不计数，也不重置）
...
出现得比 hedge 频繁得多。[`test_doc_sync.py`](tests/test_doc_sync.py) 会从
...
每条含完成声明的回复都以这个块收尾。字段名**就是** Stop 钩子的检测标记：
...
cc-enforcer:
  ...
  convergence:
    re-trigger: "$ python -m unittest → Ran 765 tests, OK"
    boundary case: ...
    existing tests: ...
...
- **改动大小是相对文件量的，不是相对一个常量。**
  ...
  目标文件的多少，所以整体重写一个 30 行模块算系统式，而不是把该文件锁死一整个
  会话。
...
- **并发是被处理过的，而且有数字。** 每次钩子调用都是独立 OS 进程，Claude Code
  ...
  10 次记录的 read 丢 2–3 次。
- **一个上限只能有一个含义。** tldr 上限计的是**显示列**而不是码位，因为一个 CJK
  字符占两列：160 码位在英文里约等于一句话，在中文里约等于两个段落。
...
而卸载之后它什么都强制不了。强制是尽力而为，**不是安全边界**——它是为了挡住一个
偷懒的 agent，不是一个敌对的 agent。

**宁可漏报不误报。** 漏一次违规的代价是一次偷懒的编辑；误报一次的代价是浪费一轮
并侵蚀对闸门的信任。够不到的地方写进规则文件，而不是悄悄打补丁盖掉。

**规则是产品，钩子只是一层适配器。** `rules/` 是零运行时依赖的纯 Markdown，
`cat rules/*.md` 是有文档的安装路径。所以规则文件描述的逃生口若比钩子实际执行的
更弱，就是已发布的缺陷：它等于给每一个非 Claude Code 的消费者发了更弱的纪律。
...
- **版本门** —— 每一个版本指针、**两个** README 的徽章、CHANGELOG 最新标题，都由
  一个**封闭集**钉死到 `plugin.json`；**每一个**已发布的 CHANGELOG 标题还要与
  `git tag` 对账；
- **文档门** —— 本 README 里被钉住的每个数字都在测试时从代码派生，外加双向的清单
  检查（树里列着一个已删除的文件，同样是漂移）；
- **i18n 门** —— 每份翻译都与英文骨架做结构比对，包括 DENY 行的 token 奇偶；
- **demo 门** —— 两个 README 嵌的前后对比图，每次都用真钩子重新渲染后逐字节比对。
...
│   └── zh/                      # 中文翻译，结构由 CI 门禁锁定
├── prompts/                     # SessionStart 合约 + 每轮提醒（含 zh/）
├── hooks/
│   ├── hooks.json               # 事件 → 脚本的接线
│   └── scripts/
...
│           ├── envfile.py       # 功能：CLAUDE_ENV_FILE 去重卫生
...
├── docs/                        # 索引 + ARCHITECTURE、CONTRIBUTING、RULES、EDICTS、I18N
├── demo/                        # 同一任务跑两遍 —— 第五节那两张图
...
└── tests/                       # 765 个测试（python -m unittest discover tests）
    ...
    ├── test_startup_cost.py     #   每个钩子的常规路径都 import 了什么；正则懒编译
...
本插件用自己的规则管自己的开发 —— 在改它的时候，预期会被它拒绝。仓库的规矩、
本地检查与发布清单在 [`docs/CONTRIBUTING.md`](docs/CONTRIBUTING.md)；改任何东西
之前要查的连带文件表在 [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) §8。版本
